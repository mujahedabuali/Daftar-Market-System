...
class Sold(ck.CTkFrame):
    def __init__(self, parent,login_page_instance):
        ...
        self.edit_button2 = ck.CTkButton(button_frame2, text="تعديل الاسم",font=ck.CTkFont(size=20,weight="bold"),width=10,command=self.printAssist)
        self.edit_button2.grid(row=0, column=1, padx=10)
        self.edit_button2.configure(state="disabled")

        self.delete_button2 = ck.CTkButton(button_frame2, text="حذف",fg_color="red",font=ck.CTkFont(size=20,weight="bold"),width=10,command=self.printAssist)
    ...
    def show_detial_info(self):
        ...
        sql_query = """
                    SELECT Products.ProductID, Products.ProductName, Products.sell_Price,OrderDetails.discount,OrderDetails.Quantity,OrderDetails.Subtotal
                    FROM Orders
